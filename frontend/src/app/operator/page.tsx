...
export default function OperatorPage() {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] w-full bg-[#0a0a0a] text-white p-3.5 sm:p-6 lg:p-10 font-sans">
      <div className="max-w-6xl mx-auto flex flex-col gap-4 sm:gap-6">
        {/* Top Header / Status Section */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 sm:pb-6 border-b border-white/10">
          <div>
            ...
            <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-white">Operations Agent</h1>
          ...
          <div className="flex flex-wrap sm:flex-nowrap items-center gap-2.5 sm:gap-3">
            ...
            <button
              ...
              disabled={evaluating}
              className="inline-flex items-center justify-center gap-2 px-3.5 py-1.5 rounded-lg bg-white hover:bg-zinc-200 disabled:opacity-50 text-black text-xs font-semibold transition shrink-0"
            >
        ...
        <div className="px-3.5 sm:px-4 py-2.5 sm:py-3 rounded-lg bg-white/3 border border-white/10 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 text-xs text-zinc-300">
          <span className="text-[11px] sm:text-xs font-semibold text-white uppercase tracking-wider bg-white/10 px-2 py-0.5 rounded w-fit shrink-0">
            Goal
          </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6">
          ...
          <div className="lg:col-span-5 flex flex-col gap-4 sm:gap-6">
                ...
                <div className="border-t border-white/5 pt-3">
                  ...
                  <div className="space-y-2">
                    {dlqItems.slice(0, 3).map((job) => (
                      <div key={job.jobId} className="bg-amber-500/5 border border-amber-500/10 rounded-md p-2.5 text-xs">
                        <div className="flex flex-wrap sm:flex-nowrap justify-between items-center gap-1 text-zinc-200 font-medium">
                        ...
                        <p className="text-[11px] text-zinc-400 break-words mt-1">{job.errorMessage}</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="bg-[#111111] border border-white/10 rounded-xl p-4 sm:p-5 flex-1 flex flex-col">
              <h2 className="text-xs sm:text-sm font-semibold text-white mb-3 flex items-center gap-2">
                <ClockIcon className="w-4 h-4 text-zinc-400" />
                Agent Activity Sequence
              </h2>
              <div className="space-y-2.5 overflow-y-auto max-h-64 sm:max-h-80 pr-1 flex-1">
                {activityLog.length === 0 ? (
                  ...
                  activityLog.map((act) => (
                    <div key={act.id} className="text-xs border-l-2 pl-3 py-1 border-zinc-800">
                      ...
                      <p className="text-zinc-300 leading-snug break-words">{act.summary}</p>
                      {act.detail && (
                        <p className="text-[11px] text-zinc-500 font-mono mt-0.5 break-all">{act.detail}</p>
                      )}
                    </div>
                  ))
                )}
          ...
          <div className="lg:col-span-7 flex flex-col gap-4 sm:gap-6">
            ...
            <div className="bg-[#111111] border border-white/10 rounded-xl p-4 sm:p-5">
              ...
              <p className="text-xs text-zinc-300 leading-relaxed bg-black/50 p-3 sm:p-3.5 rounded-lg border border-white/5 mb-3 break-words">
                {decision ? decision.reason : "Evaluating telemetry against persistent goal..."}
              </p>

              {decision?.action && decision.action.tool !== "none" && (
                <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-400 bg-black/30 px-3 py-2 rounded-lg border border-white/5">
            ...
            {workspaceProposal && (
              <div className="bg-[#111111] border-2 border-white/20 rounded-xl p-4 sm:p-5 shadow-xl">
                <div className="flex items-center gap-2.5 mb-3 text-zinc-200">
                  <CubeTransparentIcon className="w-5 h-5 text-zinc-400 shrink-0" />
                  <h3 className="text-xs sm:text-sm font-bold text-white">Proposal: Provision Processing Workspace</h3>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
                  <div className="bg-black/60 p-2.5 rounded-lg border border-white/5">
                    ...
                    <span className="text-xs font-medium text-white truncate block">{workspaceProposal.name}</span>
                  </div>
                  <div className="bg-black/60 p-2.5 rounded-lg border border-white/5">
                    <span className="text-[10px] text-zinc-500 block">Environment</span>
                    <span className="text-xs font-medium text-white truncate block">{workspaceProposal.environment}</span>
                  </div>
                  <div className="bg-black/60 p-2.5 rounded-lg border border-white/5">
                    <span className="text-[10px] text-zinc-500 block">Region</span>
                    <span className="text-xs font-medium text-white truncate block">{workspaceProposal.region}</span>
                  </div>
                  <div className="bg-black/60 p-2.5 rounded-lg border border-white/5">
                    <span className="text-[10px] text-zinc-500 block">Concurrency</span>
                    <span className="text-xs font-medium text-white truncate block">{workspaceProposal.workerConcurrency} workers</span>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 sm:gap-3">
                  ...
                  <button
                    ...
                    disabled={approving}
                    className="px-4 py-2 bg-zinc-800 hover:bg-zinc-700 disabled:opacity-50 text-zinc-300 text-xs font-medium rounded-lg transition border border-white/5 text-center"
                  >
                    Dismiss
                  </button>
                </div>
            ...
            {activeTx && (
              <div className="bg-[#111111] border border-white/10 rounded-xl p-4 sm:p-5">
                <div className="flex flex-wrap sm:flex-nowrap items-center justify-between gap-2 mb-3 border-b border-white/10 pb-3">
                  <div className="flex items-center gap-2 min-w-0">
                    <ShieldCheckIcon className="w-5 h-5 text-zinc-400 shrink-0" />
                    <div className="min-w-0">
                      <h3 className="text-xs sm:text-sm font-bold text-white truncate">
                        MCPx Orchestration: {activeTx.workspaceName}
                      </h3>
                      <span className="text-[10px] sm:text-[11px] text-zinc-500 font-mono truncate block">
                  ...
                  <a
                    ...
                    className="flex items-center gap-1 text-xs text-zinc-400 hover:text-white font-medium transition shrink-0"
                  >
                ...
                <div className="space-y-2 mb-4">
                  {activeTx.nodes.length === 0 ? (
                    ...
                    activeTx.nodes.map((node) => (
                      <div
                        key={node.id}
                        className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-0 p-2.5 rounded-lg bg-black/60 border border-white/5 text-xs"
                      >
                        <div className="flex items-center gap-2 truncate">
                          <span className="font-medium text-zinc-200 truncate">{node.fileflowLabel}</span>
                          <span className="text-[10px] text-zinc-500 font-mono shrink-0">({node.id})</span>
                        </div>
                        <span
                          className={`text-[11px] font-medium px-2 py-0.5 rounded border self-start sm:self-auto ${node.state === "SUCCEEDED"
                ...
                {activeTx.isAwaitingApproval && !activeTx.compensationDone && (
                  <div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-3 sm:p-3.5 mt-3">
            ...
            <div className="bg-[#111111] border border-white/10 rounded-xl p-2.5 sm:p-3 flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
              <input
                type="text"
                value={inputPrompt}
                ...
                className="flex-1 bg-black border border-white/10 rounded-lg px-3.5 py-2 text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-white/30 transition min-w-0"
              />
              <button
                onClick={handleManualSubmit}
                disabled={manualLoading || !inputPrompt.trim()}
                className="px-4 py-2 bg-white hover:bg-zinc-200 disabled:opacity-30 text-black rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 transition shrink-0"
              >
